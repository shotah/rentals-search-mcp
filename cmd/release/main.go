...
func run(args []string) int {
	...
	bump := fs.String("bump", "patch", "Version bump: patch, minor, or major (ignored if -version is set)")
	version := fs.String("version", "", "Explicit version (e.g. v0.2.0)")
	dryRun := fs.Bool("dry-run", false, "Print actions without tagging or pushing")
	skipPush := fs.Bool("skip-push", false, "Create local tag/commit but do not push")
	allowDirty := fs.Bool("allow-dirty", false, "Allow uncommitted changes (not recommended)")
...
// latestTag is kept as an alias for tests that call the old name.
func latestTag() string { return latestSemverTag() }
